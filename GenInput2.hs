...
-- Generate input file
genInputFile :: String -> String -> FilePath -> String -> IO ()
genInputFile header label geom footer = do
  geoms  <- readFile $ geom ++ ".xyz"
  let
    --fileName  = geom ++ label ++ ".inp" -- old
    fileName  = (map (\x -> if x == '.' then '-' else x) $ geom ++ label) ++ ".inp"
...
main = do
  ...
  if length args == 3 then do
    ...
    putStrLn $ "Footer file      ... " ++ (show (args !! 2))     
    ...
    putStrLn ""    
    putStrLn " Usage:"
    putStrLn "       GenInput HeaderFile GeomDirec FooterFile"    
    putStrLn " "
    putStrLn " Arguments:"
    ...
    putStrLn "       FooterFile - File which contains footer part of the input"    
    putStrLn ""
  ...
  footers <- readFile footerName
  putStrLn " Footer files .. \n"
  putStrLn " ==================================="
  putStr footers
  putStrLn " ==================================="
  putStrLn ""    
  ...
  mapM (\x -> genInputFile headers myFooter x footers) xyzFiles
