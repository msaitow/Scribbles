...
std::string generate_input(const double intern_dist, const std::string name_base, const bool isFirst=false);
...
int main(int argc, char* argv[])
{
  ...
  for(std::vector<double>::const_iterator l = lengths.begin();l != lengths.end();++l){
    ...
    std::string name(generate_input(*l, n_base, (l==lengths.begin())));
    std::string copyChkx   ( (l!=lengths.begin()) ? ("cp " + prevName + ".chkx ./" + name + ".chkx") : "" );
    std::string scf_exec   ("~/ORZSAVE3/orz/obj-opt/src/sci/scf/scf ./" + name + ".py | tee " + scf_name);
    std::string casscf_exec("~/ORZSAVE3/orz/obj-opt/src/sci/casscf/casscf ./" + name + ".py | tee " + casscf_name);
    std::string mrci_exec  ("~/ORZSAVE3/orz/obj-opt/src/sci/lct/lct ./" + name + ".py | tee " + mrci_name);    
    if(l!=lengths.begin()) system(copyChkx.c_str());
...
std::string generate_input(const double intern_dist, const std::string name_base, const bool isFirst)
{
  std::ostringstream stm;
  stm << intern_dist;
  ...
  file << "" << std::endl;
  if(isFirst==false){
  file << "scf.maxloop=0" << std::endl;
  file << "scf.guess=\"moread\"" << std::endl;
  }
