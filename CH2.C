{
  ...
  c1 = new TCanvas("c1","Spin Chains:CH2",200,10,700,500);
  c1->SetGrid();
  
  TGraph *scale_100 = new TGraph("CH2.latest.csv", "%lg %lg %*s %*s %*s", ",");
  ...
  TGraph *scale_050 = new TGraph("CH2.latest.csv", "%lg %*s %lg %*s %*s", ",");
  ...
  TGraph *scale_033 = new TGraph("CH2.latest.csv", "%lg %*s %*s %lg %*s", ",");
  ...
  TGraph *scale_010 = new TGraph("CH2.latest.csv", "%lg %*s %*s %*s %lg", ",");
  ...
  TMultiGraph *mg = new TMultiGraph("", ";Number of CH2 Monomers; Correlation Energy Recovered (%)");
  mg->Add(scale_100);
  mg->Add(scale_050);
  mg->Add(scale_033);
  mg->Add(scale_010); 
