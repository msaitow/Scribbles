{
  ...
  c1 = new TCanvas("Verda-DO-N4","TCutDO",200,10,700,500);
  c1->SetGrid();
  c1->SetLogx();  
  
  TGraph *scale_100 = new TGraph("DO-Verdazyl.csv", "%lg %lg %*s %*s %*s", ",");
...
///  scale_100->GetXaxis()->SetLimits(1,10);
  ...
  TGraph *scale_050 = new TGraph("DO-Verdazyl.csv", "%lg %*s %lg %*s %*s", ",");
  scale_050->SetMarkerColor(kBlue);
  scale_050->SetLineColor(kBlue);  
...
///  scale_050->GetXaxis()->SetLimits(1,10);
  ...
  TGraph *scale_033 = new TGraph("DO-Verdazyl.csv", "%lg %*s %*s %lg %*s", ",");
  scale_033->SetMarkerColor(kBlue);
  scale_033->SetLineColor(kBlue);    
...
///  scale_033->GetXaxis()->SetLimits(1,10);
  ...
  TGraph *scale_010 = new TGraph("DO-Verdazyl.csv", "%lg %*s %*s %*s %lg", ",");
  scale_010->SetMarkerColor(kBlue);
  scale_010->SetLineColor(kBlue);    
...
///  scale_010->GetXaxis()->SetLimits(1,10);
  ...
  TMultiGraph *mg = new TMultiGraph("", ";TCutDO; Isotropic Hyperfine Coupling Constants in MHz");
//  mg->Add(scale_100);
//  mg->Add(scale_050);
//  mg->Add(scale_033);
  mg->Add(scale_010); 
