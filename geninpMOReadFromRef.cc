...
const std::string basis("def2-QZVPP");
...
int main(int argc, char* argv[])
{
  ...
  std::string RefDir   = "/home/00/saitow/Ref_Chkx_N2_def2_QZVPP/";
  
  for(std::vector<double>::const_iterator l = lengths.begin();l != lengths.end();++l){
    std::ostringstream length;
    length << *l;
    std::ostringstream stm;
    stm << *l << "_" << boost::format("%e_%e") % _tCutPNO % _tCutPairs;    
    std::string method("");
    method += (_doCEPT2 ? "_CEPT2_" : "_CASPT2_") + basis + "_";
    const std::string scf_name   ("./scf_r_" + stm.str() + "_.out");
    const std::string casscf_name("./casscf_r_" + stm.str() + "_.out");
    const std::string mrci_name  ("./lct_r_" + stm.str() + "_.out");    
    ...
    std::string copyChkx   ("cp " + RefDir + length.str() + ".chkx ./" + name + ".chkx");
    std::string scf_exec   ("~/ORZSAVE3/orz/obj-optX/src/sci/scf/scf ./" + name + ".py | tee " + scf_name);
    std::string casscf_exec("~/ORZSAVE3/orz/obj-optX/src/sci/casscf/casscf ./" + name + ".py | tee " + casscf_name);
    std::string mrci_exec  ("~/ORZSAVE3/orz/obj-optX/src/sci/lct/lct ./" + name + ".py | tee " + mrci_name);
    cout << "HAGEEEEE\n" << copyChkx << endl;///
    system(scf_exec.c_str());
    system(copyChkx.c_str());
...
std::string generate_input(const double intern_dist, const std::string name_base)
{
  ...
  std::string name(name_base + "_" + stm.str() );
  std::string fileName(name + ".py");
  ...
  file << "" << std::endl;
  //file << "scf.guess=\"moread2\"" << std::endl;
  file << "scf.maxloop=1" << std::endl;
  file << "nstate=1                        " << std::endl;
  file << "scf.dodirect = False                                                        " << std::endl;
  file << "#scf.calc_ao_tei = False                                                      " << std::endl;
  file << "scf.show_orbitals = False                                                    " << std::endl;
  ...
  file << "                                                                            " << std::endl;
  file << "qmolecule_RI_C = qmolecule_RI_JK" << std::endl;
  file << "" << std::endl;
  ...
  file << "icmr.frozen = [0,1]" << std::endl;
  file << "icmr.closed = [2,3]" << std::endl;
  ...
  file << "icmr.pt.lvl_shift = 0.0" << std::endl;
  file << "icmr.pt.toliter   = 1e-16" << std::endl;
  file << "" << std::endl;
  ...
  file << "lct.NewTrafo    = True" << std::endl;
  file << "lct.Make4EXT    = False" << std::endl;
  file << "lct.Store4EXTOnDisk = True" << std::endl;
  file << "" << std::endl;
